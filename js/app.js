...
function layout(title, content, { back = false, active = '' } = {}) {
...
function renderTalk() {
  ...
  app.innerHTML = layout('', `<div class="talk-head"><div><span class="mode-pill ${state.meta.mode}">${state.meta.mode.toUpperCase()}</span><h1>${escapeHTML(state.meta.emoji)} ${escapeHTML(state.meta.title)}</h1></div><span class="progress-label">${state.turn} / ${turnCount(state.scene)}</span></div><div class="progress-track"><span style="width:${Math.round(state.turn / turnCount(state.scene) * 100)}%"></span></div><section class="conversation"><div class="scene-backdrop ${escapeHTML(state.meta.category)}"><div class="avatar-glow"></div><div id="avatar" class="avatar" role="img"></div><span class="name-tag">${escapeHTML(profile.name)}</span></div><div class="speech-card"><span class="speaker">${escapeHTML(profile.name)} says</span><p class="english" ${state.settings.captions ? '' : 'aria-label="字幕は設定で非表示"'}>${state.settings.captions ? escapeHTML(node.prompt) : '•••'}</p><button class="replay" data-action="replay" aria-label="もう一度聞く">🔊 Replay</button></div></section><section class="reply"><div class="reply-title"><span>YOUR TURN</span><p>${escapeHTML(node.task)}</p></div>${state.transcript ? `<div class="transcript"><small>聞き取った英語</small><p>${escapeHTML(state.transcript)}</p></div>` : ''}${feedbackHTML()}${state.status === 'checking' ? '<p class="checking">⏳ Checking...</p>' : ''}${!success ? `<button class="mic-button ${state.status === 'listening' ? 'recording' : ''}" data-action="mic" ${state.status === 'checking' ? 'disabled' : ''}>${state.status === 'listening' ? '🔴 Listening...' : '🎤 話す'}</button><form id="type-form" class="type-form"><label for="typed">${speechSupported() ? '声が使えないときは文字入力' : '文字入力で練習'}</label><div><input id="typed" name="typed" type="text" lang="en" autocapitalize="sentences" autocomplete="off" placeholder="Type your reply in English"><button type="submit">判定</button></div></form><div class="help-row"><button data-action="retry">もう一度</button><button data-action="hint">回答例を見る</button><button data-action="skip">このまま進む</button></div>${state.hintShown ? `<div class="hint">例：${escapeHTML(node.examples[0])}</div>` : ''}` : '<p class="moving">次の会話へ…</p>'}</section><p class="talk-credit">Made by YUU · v1.0.0</p>`, { back: true });
